...
const wordFormSchema = z.object({
  pl: z.string().min(1, "Słowo po polsku jest wymagane").max(200, "Słowo po polsku jest za długie (max 200 znaków)"),
  en: z
    .string()
 ...
 */
export function WordEditorModal({ open, onOpenChange, mode, initialValues, onSubmit }: WordEditorModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>{mode === "create" ? "Dodaj słówko" : "Edytuj słówko"}</DialogTitle>
          <DialogDescription>
            {mode === "create"
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              ...
              />
              {errors.pl && <p className="text-sm text-red-500">{errors.pl}</p>}
            </div>
            ...
            <div className="grid gap-2">
              ...
              />
              {errors.en && <p className="text-sm text-red-500">{errors.en}</p>}
            </div>
          </div>
