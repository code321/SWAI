...
export async function listSets(
  ...
  let queryBuilder = supabase.from("sets").select("id, name, level, words_count, created_at").eq("user_id", userId);
  ...
  if (cursor) {
    ...
    if (sort === "created_at_desc") {
      ...
      queryBuilder = queryBuilder.or(
        `created_at.lt.${timestampOrName},and(created_at.eq.${timestampOrName},id.lt.${uuid})`
      );
    } else {
