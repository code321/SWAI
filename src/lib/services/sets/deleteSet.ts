 ...
 */
export type DeleteSetResult = { success: true } | { success: false; error: "NOT_FOUND" | "ACTIVE_SESSION" };
 ...
 */
export async function deleteSet(supabase: SupabaseClient, userId: string, setId: string): Promise<DeleteSetResult> {
  ...
  // - attempts
  // - ratings
  const { error: deleteError } = await supabase.from("sets").delete().eq("id", setId).eq("user_id", userId);
